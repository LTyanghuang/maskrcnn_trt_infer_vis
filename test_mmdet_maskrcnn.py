...
class TestModelConvertor(unittest.TestCase):
    ...
    def test_mmdet_maskrcnn_loadtrt(self):
        bsize = 1
        convertor = ModelConvertor()
        img_readpath = '/home/yhuang/datasets/coco/images/val2017/'
        ...
        for trt_model_path in [ 
            ...
            for relimgpath in [
                ...
                stime = time.time()
                for iloop in range(0, nloop):
                    _, _, _, bboxes, scores, labels, _, masks = convertor.predict(batch_in)
                etime = time.time()
                ...
                for score_thr in [0.3, 0.4]:
                    ...
                    image_array = cv2.resize(src_img, (800, 800))
