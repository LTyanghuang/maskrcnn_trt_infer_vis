...
def preprocess_coco_mmdet_maskrcnn(image, channels=3, height=224, width=224):
    """Pre-processing for COCO, mmdetection framework in detail
    Parameters
    ----------
    image: PIL.Image
...
def preprocess_voc_mmdet_ssd(image, height=224, width=224, channels=3):
    """Pre-processing for VOC, mmdetection framework in detail
    Parameters
    ----------
    image: PIL.Image
        The image resulting from PIL.Image.open(filename) to preprocess
    ...
    for i in range(img_data.shape[2]):
        # Scale each pixel to [0, 1] and normalize per channel.
        # img_data[:, :, i] = (img_data[:, :, i] / 255 - mean_vec[i]) / stddev_vec[i]
        img_data[:, :, i] = (img_data[:, :, i] - mean_vec[i])
